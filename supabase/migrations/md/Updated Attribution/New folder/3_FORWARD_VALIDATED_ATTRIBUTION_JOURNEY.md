...
**Status (MVP lock — 2026-09-08):**
V1 superseded. V2 edge model implemented and proven on same-origin hops.
V3 `events_journey` + `vt_ej_id` implemented and proven for Tier 1 / Tier 2
...
This is a known browser `localStorage` origin boundary, not a regression to "fix"
with cookies, `vt_sid` packing, or probabilistic matching.

**Tier 3** (YouTube / static description → external redirect without VSTRK JS)
remains a hard boundary (§19D-Tier3).

**Tracking-domain product rules:** see companion doc
`4_TRACKING_DOMAIN_MVP_RULES.md` and §22 below.

**Conversion pages may differ from the tracking host** (e.g. tracking on
`go.ali.com`, thank-you on `ali.com`) — that is expected Tier-2 `vt_*` / pixel
handoff, not a journey-continuity failure.

**Phase 7** legacy cleanup remains a separate, not-yet-approved decision.
All items in §17B remain RESOLVED.
...
- **Phase 7 — STILL NOT STARTED.** Originally deferred pending V2, then pending V3. V3 (Section 19,
  persistent server-side journey history via `events_journey`) is now implemented and real-world
  proven for VSTRK-controlled surfaces (Tier 1 / Tier 2), including same-host
  **A → B → C → Newsletter** (terminal Campaign Element node). Tier 3 is a locked, accepted boundary,
  not a blocker (§19D-Tier3). **Cross-tracking-origin continuous journey is MVP-deferred** (§22).
  Phase 7 legacy cleanup (removing `FT_*` code / fallback code) is a
  separate, still-unapproved decision — V3 being implemented does not by itself authorize cleanup.
...
## 17B. Decisions — V3 (ALL RESOLVED; kept for historical record)

See Section 19 for full context on each.

1. ~~Whether `events_journey` rows are written synchronously (awaited, before redirect)~~ —
   RESOLVED: yes. `logRedirectEvent()` is now awaited in `Track.tsx` **before** the redirect
   (Step 6B), so the real inserted `events.id` is available for `events_journey.event_ids`. The
   old fire-and-forget post-redirect call was removed entirely.
2. ~~Whether `redirect_link_id` is sufficient as the deterministic key~~ — RESOLVED: no.
   `redirect_link_id` is shared across every visitor who clicks that link, so it can't identify
   one visitor's journey. A new field, `events_journey_id` (propagated end-to-end as `vt_ej_id`),
   was added instead — the exact primary key of the `events_journey` row for that click.
   `api/pixel.ts` looks up `events_journey` by `id = events_journey_id`, falling back to the
   client-supplied `journey` only if that lookup is empty or absent. See §19F.
3. ~~Exact `events_journey` schema~~ — RESOLVED: implemented matching the Section 19B draft, with
   `journey_id`, `event_ids`, and `journey_snapshot` all `NOT NULL`. RLS is enabled with an
   explicit `INSERT` policy for `anon`/`authenticated` (Track.tsx writes this table directly from
   the client). See §19B.
4. ~~Confirmation that no deterministic mechanism exists to carry a journey across a static
   YouTube-description hop~~ — RESOLVED: confirmed, accepted, and locked. See §19D-Tier3.

---

## 19. V3 — Persistent Server-Side Journey History (IMPLEMENTED for Tier 1 / Tier 2, real-world proven)
...
Implemented schema (migration `20260906_PixelPurchaseAttributions.sql`):

events_journey
id uuid, pk, default gen_random_uuid()
journey_id uuid NOT NULL -- correlation/grouping only, see 19D — NOT the reconstruction mechanism
event_ids jsonb NOT NULL -- ordered array of events.id, exact — never timestamp-matched
journey_snapshot jsonb NOT NULL -- complete JourneyNode[] as of this click
redirect_link_id uuid, references redirect_links(id) on delete set null -- this click's redirect
created_at timestamptz NOT NULL default now()

Indexes: `idx_events_journey_journey_id`, `idx_events_journey_redirect_link_id`. RLS is enabled,
with an explicit `INSERT` policy for `anon`/`authenticated` (`Track.tsx` writes this table
directly from the client, unauthenticated). No separate join table for `event_ids` — a concrete
query need never materialized one.

**The `id` column — not `journey_id` — is the actual deterministic purchase-time lookup key**,
propagated end-to-end as `vt_ej_id` → `events_journey_id`. See §19D and §19F.

### 19C. Exact `events.id` capture — no timestamp matching

`redirects.ts`'s `logRedirectEvent()` now returns `Promise<string | null>` instead of `void`. The
insert itself was also moved off a bare `.insert()` on `events` and into a new `SECURITY DEFINER`
RPC, `log_redirect_event()`, which performs the insert server-side and returns the new row's `id`
via `RETURNING id`. Functionally this is the same outcome the draft asked for — `logRedirectEvent()`
resolves to the new `events.id` or `null` on failure.

`Track.tsx` now calls `logRedirectEvent` **awaited, before** `window.location.href` is set (Step
6B) — the old fire-and-forget post-redirect call was removed entirely. This was the accepted
latency tradeoff; decision 17B-1 is resolved as "yes, synchronous."
...
Implemented in `tracker.ts`: `getJourneyId()`/`setJourneyId()` persist `journey_id` to
localStorage (`yt_tracker_journey_id`); `getEventIds()`/`setEventIds()` do the same for `event_ids`
(`yt_tracker_event_ids`). `appendJourneyNode()` calls `setJourneyId(crypto.randomUUID())` every
time it starts a fresh journey (empty, or a failed continuation check) — same reset points as
`journey` itself. `Track.tsx` extends `event_ids` from the prior stored list when the journey is
continuing (`journey.length > 1`), or resets it to just the current hop's `events.id` when the
journey just started/reset (`journey.length === 1`).

Continuity tiers for `journey_id` (same tiers apply to the V2 browser journey itself and to
`events_journey_id`/`vt_ej_id` propagation):
- **Tier 1 — same-origin localStorage.** Deterministic. Covers `57y8 → d3es` (Section 6B Part 1).
- **Tier 2 — controlled URL propagation** (`vt_journey` / `vt_ej_id`, same pattern). Only helps
  when VSTRK controls the very next page — does not help across a YouTube description.
- **Tier 3 — YouTube watch page → static description link → external VSTRK redirect. HARD
  ATTRIBUTION BOUNDARY. Accepted and locked.** See §19D-Tier3.

### 19D-Tier3. Explicit Tier-3 limitation (LOCKED — do not "fix" later with unsafe workarounds)

**Surface:**
```
youtube.com/watch?v=...  (Video C)
  ↓ static link in YouTube description
https://go.kaksidigitals.com/LnMI  (or any external VSTRK redirect)
```

**Why no deterministic hand-off exists:**
The user is on YouTube's own watch page. VSTRK JavaScript (`Track.tsx`, `tracker.ts`,
`installationHelpers.ts`) does **not** run there. Description links are static HTML written by the
creator — YouTube does not copy query parameters from the current watch URL into those links. When
the browser later hits the external VSTRK host, it is a different origin: localStorage is empty and
the incoming URL has no `vt_ej_id`/`vt_journey`. The click therefore correctly starts a new journey.

**Explicitly rejected (never implement):**
- IP / user-agent / timing / fingerprint matching
- "Most recent compatible journey" or any probabilistic cross-user join
- Permanently baking a per-visitor `events_journey.id` into a reusable `redirect_links.destination_url`
- Adding `events.events_journey_id` solely to paper over this hop (the column cannot receive a
  deterministic value at Tier 3)

**What remains correct and complete:**
- The A → B → C persistent journey on VSTRK-controlled surfaces (Tier 1 / Tier 2) is implemented
  and real-world proven.
- The earlier `events_journey` row for A → B → C is never mutated or deleted when a later Tier-3
  click occurs.
- Newsletter (or any post-Tier-3) conversion continues to use the existing lightweight
  attribution/fallback path in `api/pixel.ts` (`forward_journey` when a journey is present,
  otherwise `redirect_link_id` / `unresolved`). No change required.

**When to revisit:**
Only if VSTRK later controls the page containing the Video C → Newsletter CTA (e.g. a
VSTRK-hosted landing page or embed that can rewrite the CTA URL with the current
`events_journey.id` before the click). Until that surface exists, treat Tier 3 as a hard boundary
and do not add schema or matching logic to bridge it.
...
### 19F. Purchase-time retrieval (IMPLEMENTED — differs from the original draft above)

Decision 17B-2 did **not** land on `redirect_link_id` as the lookup key — `redirect_link_id` is
shared across every visitor who clicks that link, so it can't identify one visitor's specific
journey. Instead, `api/pixel.ts` accepts a new `events_journey_id` field in the pixel POST body,
propagated end-to-end as `vt_ej_id` on the redirect URL → `installationHelpers.ts` → payload, and
looks up `events_journey` **by its own primary key**, `id = events_journey_id` — never by
`redirect_link_id` and never by `journey_id` (§19D covers why `journey_id` isn't the lookup key
either: one `journey_id` can span multiple rows).

If that row is found, its `journey_snapshot` is the deterministic answer — `match_method =
'forward_journey'`, `resolution_status = 'resolved'` — and `journey_display` is rebuilt from it by
joining `redirect_links.token`s. If no `events_journey_id` was sent, or the lookup finds nothing
(e.g. a legacy cached script, or the Tier 3 gap swallowed the fuller history), `api/pixel.ts` falls
back to the client-supplied `parsedJourney` (the original V2 behavior), then to
`first_touch_redirect_link_id`, exactly as before — not a heuristic substitute.

This fixes Section 6B's specific bug as a side effect for Tier 1 / Tier 2 hops, since resolution no
longer depends on anything surviving in the browser by purchase time — only on the `events_journey`
row existing server-side.
...
4. **V3 — snapshot-per-row `events_journey`, `event_ids` for exact deterministic linkage to
   `events`, `journey_id` demoted to correlation-only** — IMPLEMENTED for all VSTRK-controlled
   surfaces (Tier 1 / Tier 2). Real-world proven for A → B → C. Tier-3 (YouTube watch page →
   static description link → external redirect) is an accepted hard boundary; see §19D-Tier3.
   Do not add `events.events_journey_id` or any probabilistic bridge to "fix" Tier 3.

**Do not read Sections 2–7 (V1) as current design.** They are retained for historical/testing
context only, clearly superseded by Section 8 (V2, implemented) and Section 19 (V3, implemented
for controlled surfaces; Tier 3 locked).
...
## 22. MVP Locks (2026-09-08) — Journey Continuity & Domains

This section records product/architecture locks decided after real-world tests
of same-host A → B → C → Newsletter and failed cross-origin custom-domain hops.

### 22A. What is proven and complete for MVP

1. **Same exact tracking hostname** for all VSTRK `Track.tsx` hops in a funnel:
   ```
   go.example.com/t1 → go.example.com/t2 → go.example.com/t3 → go.example.com/newsletter
   ```
   (or the same pattern on `www.vstrk.com`) preserves one `journey_id`, grows
   `events_journey` snapshots, and supports a terminal Newsletter node with
   `destination_video_id = null`.

2. **V2 edge continuation** (`previous.destination_video_id === current.video_id`)
   remains the primary rule; asset-membership remains fallback. Organization ID
   is never a journey boundary.

3. **Terminal non-video nodes** (Campaign Element Asset / Resource) are valid
   journey end nodes: still appended; still eligible for an `events_journey`
   snapshot when same-origin context exists. They do not mean "skip journey."

4. **Customer conversion / thank-you pages** may live on a different host than
   the tracking domain (e.g. `go.ali.com` tracking → `ali.com/newsletter-thankyou`).
   Attribution uses Tier-2 URL params (`vt_sid`, `vt_ej_id`, `vt_journey`, …) and
   destination pixel / global website script — **not** tracking-host localStorage
   continuity. This is expected and supported.

### 22B. Explicitly deferred (do not block MVP)

1. Continuous journey across **different tracking origins**
   (`go.ali.com` → `go.nike.com`, `www.vstrk.com` → `go.customer.com`,
   `nike.vstrk.com` → `www.vstrk.com`).

2. Tier-3 handoff through platforms VSTRK does not control (YouTube description
   static links). No cookies, no `vt_sid` composite hacks, no probabilistic
   device matching to "fix" this.

3. Phase 7 removal of legacy `FT_*` keys / heavy offline resolvers.

### 22C. Tracking-domain product locks (summary)

Full rules live in **`4_TRACKING_DOMAIN_MVP_RULES.md`**. Short form:

| Rule | Lock |
|------|------|
| Continuity unit | **Exact hostname (origin)**, not "same parent domain" or "same org" |
| Multiple branded domains per org | **Allowed** |
| One campaign → one domain forever | **Not required** for MVP |
| One **funnel / public asset chain** → one hostname | **Required** for continuous journey |
| Each `redirect_links` row | Permanently snapshots `tracking_hostname` |
| Promote another user's asset | Reuse **asset's stable tracking URL/host**; do not rewrite to promoter's different origin |
| Branded `slug.vstrk.com` | OK only if **entire** funnel stays on that single host |

### 22D. Code posture for MVP

- **Do not** reopen Track.tsx / `tracker.ts` journey engine for cross-origin continuity.
- **Do not** add cookies or change `vt_sid` for journey correlation without a new explicit decision.
- Optional follow-up (separate from domain policy): verify thank-you pixel always receives
  `vt_ej_id` on same-host Newsletter flows so purchase → `events_journey` lookup is deterministic.
- `pixel_purchases` need not store journey snapshot; `events_journey_id` in pixel body /
  `pixel_purchase_attributions` remains the attribution path per §12 / §19F.

### 22E. Rules for future sessions (additive to §21)

1. Read this document **and** `4_TRACKING_DOMAIN_MVP_RULES.md` before domain or journey work.
2. Do not treat "same registrable domain" or "same Supabase organization" as same origin.
3. Do not mix tracking hosts inside one content journey and call it a journey bug.
4. Wait for explicit approval before Phase 7 or cross-origin journey transport designs.
