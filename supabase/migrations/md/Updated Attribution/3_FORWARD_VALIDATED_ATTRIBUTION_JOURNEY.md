...
**Status: V1 implemented (Phases 1–6, source-oriented, superseded). V2 implemented (edge-based destination resolution, Phases 1–6 extended) and real-world tested. V3 implemented (persistent server-side journey history via `events_journey`, keyed by `events_journey_id`/`vt_ej_id`) for all VSTRK-controlled surfaces (Tier 1 / Tier 2) and real-world proven for A → B → C. Tier 3 (YouTube watch page → static description link → external VSTRK redirect) is an accepted hard attribution boundary — see §19D-Tier3. All items in §17B are RESOLVED. Persistent journey is complete for surfaces VSTRK controls; Phase 7 legacy cleanup remains a separate, not-yet-approved decision.**
...
- **Phase 7 — STILL NOT STARTED.** Originally deferred pending V2, then pending V3. V3 (Section 19,
  ...
  proven for VSTRK-controlled surfaces (Tier 1 / Tier 2). Tier 3 is a locked, accepted boundary,
  not a blocker (§19D-Tier3). Phase 7 legacy cleanup (removing `FT_*` code / fallback code) is a
