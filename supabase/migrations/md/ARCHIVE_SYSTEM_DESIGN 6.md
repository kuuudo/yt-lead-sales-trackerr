...
Status: **Assignment = LOCKED (mechanism; L1/L2 pending) · Asset = LOCKED (mechanism + L1/L2) · Video = LOCKED (mechanism + L1/L2) · Campaign = LOCKED (mechanism + L1/L2) · Promotion = LOCKED (mechanism + L1/L2 + Archive Impact)**

Investigation order: Assignment (LOCKED) → Asset (LOCKED) → Video (LOCKED mechanism + L1/L2) → Campaign (LOCKED mechanism + L1/L2) → Promotion (LOCKED mechanism + L1/L2 + Archive Impact) → Assignment L1/L2 decision → (this doc finalized) → Implementation

Do not create `archiveScope.ts`, modify pickers, modify analytics, or write any archive-related code based on this document until every entity below is marked LOCKED (including Level 1/Level 2 where applicable) and all Open Questions are answered or explicitly deferred by the product owner. Mechanism locks for all five entities are complete. Level 1/Level 2 finalization remains for Assignment only. Implementation may begin only after product owner explicitly authorizes the implementation phase.
...
| 4 | Marketplace / PromotionDetail (Level 1/Level 2 + Archive Impact design LOCKED in this doc) |

Do not decide other entities’ UX during Phase 1. Assignment Level 1/Level 2 decision remains pending.
...
**Investigation order**: Assignment → Asset → Video → Campaign → Promotion → Assignment L1/L2 decision → (doc finalized) → Implementation.

**Entity status**:
- Assignment — **LOCKED** (mechanism); Level 1/Level 2 **pending**
...
- Do not add `archive_level` / `archive_reason` / `archived_reason` / `archive_source_*` / `promotions.archived_at` columns.

**What the next conversation should do first**: Read this document in full. Assignment L1/L2 decision is the only remaining Level 1/Level 2 open item (product owner may defer or skip). Do not start implementation without explicit authorization.
...
CURRENT STATE:
Assignment = LOCKED (mechanism); L1/L2 pending
Asset = LOCKED (mechanism + L1/L2)
Video = LOCKED (mechanism + L1/L2)
Campaign = LOCKED (mechanism + L1/L2)
Promotion = LOCKED (mechanism + L1/L2 + Archive Impact)
...
Explicit bans: no promotions.archived_at, no archive_reason/level/source columns, no Promotion impact table

NEXT:
Assignment L1/L2 decision only — when product owner explicitly requests it (or defer).
