...
### Phase 1 — Asset + Central Archive Resolver: **IN PROGRESS**

**Built and typechecked (strict-mode `tsc`, zero errors) — not yet run against real data/CI:**
...
- Picker gap (`listAssetsForCampaign` / `listLibraryAssetsForAssignmentPicker`, or per the latest authorization message, `listAssetsForAssignmentPicker.ts` / `listLibraryAssetsForAssignmentPicker.ts` — **these two names don't match; flagging again, unresolved**). Not touched — the actual files haven't been provided, and this section's existing rule (don't guess at code you haven't inspected) applies here same as everywhere else in this doc.
...
Two things needed to close out Phase 1:
1. The two picker files (with confirmed, correct filenames — see "Blocked" above).
2. A quick confirm/correct on the `/campaigns/:id` route assumption used for "Go to Campaign" links in both `Assets.tsx` and `AssetDetail.tsx` — inferred from the existing `/videos/:id` pattern, not verified against the actual router.

After that, Phase 2 (Video) per the Implementation order table in §12.
