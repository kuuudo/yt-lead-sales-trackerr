...
Status: **Assignment = LOCKED (mechanism; L1/L2 pending) · Asset = LOCKED (mechanism + L1/L2) · Video = LOCKED (mechanism + L1/L2) · Campaign = LOCKED (mechanism + L1/L2) · Promotion = LOCKED (mechanism; L1/L2 pending)**

Investigation order: Assignment (LOCKED) → Asset (LOCKED) → Video (LOCKED mechanism + L1/L2) → Campaign (LOCKED mechanism + L1/L2) → Promotion L1/L2 → Assignment L1/L2 decision → (this doc finalized) → Implementation

Do not create `archiveScope.ts`, modify pickers, modify analytics, or write any archive-related code based on this document until every entity below is marked LOCKED (including Level 1/Level 2 where applicable) and all Open Questions are answered or explicitly deferred by the product owner. Mechanism locks for all five entities are complete. Level 1/Level 2 finalization remains for Promotion and Assignment. Implementation may begin only after product owner explicitly authorizes the implementation phase.
...
- Do **not** add `archived_reason` / `archive_source_*` / `archive_level` / `archive_reason` columns on entity tables (including `videos` and `campaigns`).
...
| 2 | Videos.tsx (Level 1/Level 2 design LOCKED in this doc) |
| 3 | Campaigns / CampaignDetail (Level 1/Level 2 design LOCKED in this doc) |
| 4 | Marketplace / PromotionDetail (Level 1/Level 2 design still pending) |

Do not decide other entities’ UX during Phase 1. Promotion Level 1/Level 2 must be finalized in this Design Doc before implementation of that phase.
...
| Promotion | personal only (derived impact stays in My Promotions) | `archive_ui_visibility` (type=`promotion`) | pending |
...
- Reasons remain read-time derived by the central resolver; `archive_ui_visibility` stores only Level 1 ↔ Level 2 visibility.
...
**Investigation order**: Assignment → Asset → Video → Promotion → Campaign → cross-entity Level 1/Level 2 finalization → implementation.

**Entity status (mechanism)**:
- Assignment — **LOCKED** (mechanism)
- Asset — **LOCKED** (mechanism + downstream Promotion impact policy + Level 1/Level 2)
- Video — **LOCKED** (mechanism + Level 1/Level 2 — formally written into §12)
- Promotion — **LOCKED** (mechanism); Level 1/Level 2 **not yet finalized**
- Campaign — **LOCKED** (mechanism); Level 1/Level 2 **not yet finalized**

**Level 1 / Level 2 UI status**:
- Asset — finalized and present in §12
- Video — **LOCKED** (Active exclusion of Campaign-archived Videos; Level 1 Archive Tab; Level 2 = existing modal; Hide/Unhide via `archive_ui_visibility` entity_type='video'; reasons derived by central resolver; Final Leave Rule; explicit prohibition on archive_level / archive_reason columns on videos)
- Campaign — pending
- Promotion — pending (preserve exception: derived Asset/Campaign impact does **not** put Promotion into Archive View)
- Assignment — pending (whether `entity_type='assignment'` belongs in `archive_ui_visibility` is a separate product decision)
...
- Do not reopen Assignment, Asset, Video, Promotion, or Campaign mechanisms unless a genuine contradiction is found.
...
- `archive_ui_visibility` is the sole shared persistence for Level 1 ↔ Level 2; reasons remain read-time derived.

**What the next conversation should do first**: Read this document in full. Proceed to **Campaign Level 1 / Level 2** investigation only (do not reopen Video). After Campaign is approved and written, proceed to Promotion, then Assignment decision.

**What it must not do**: Reopen any LOCKED entity without proven contradiction; write implementation code without authorization; treat recommendations as locked decisions; invent archive_level / archive_reason columns.
...
Video = LOCKED (mechanism + Level 1/Level 2 formally written into §12)
Promotion = LOCKED (mechanism); Level 1/Level 2 pending
Campaign = LOCKED (mechanism); Level 1/Level 2 pending
...
VIDEO Level 1 / Level 2 (now in §12):
- Active excludes any Video with applicable condition (own `videos.archived_at` OR parent Campaign archived)
- Level 1 = Video Archive Tab (one row, multiple reasons possible)
- Level 2 = existing ARCHIVED modal (visibility surface)
- Hide writes `archive_ui_visibility` (entity_type='video')
- Level 2 Restore = Unhide only
- Level 1 Restore = true source (Restore Video / Restore Campaign) then Final Leave Rule
- Reasons derived by central resolver; never stored on videos row
- Explicit ban: no archive_level / archive_reason / archived_reason / archive_source_* on videos
Campaign = LOCKED (mechanism + L1/L2 formally written into §12)
Promotion = LOCKED (mechanism); L1/L2 pending.


NEXT:
Promotion Level 1 / Level 2 investigation only — when product owner explicitly requests it.
