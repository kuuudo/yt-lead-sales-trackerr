...
-- SURGICAL FIX — selected_marketer_domain_id was never persisted.
--
-- Verified: this is public.create_promotion(oid 25963)'s live
-- pg_get_functiondef() output, byte-for-byte, with ONLY the three
-- ★ ADDED additions below. Confirmed via diff against the live definition
-- with those lines stripped out — zero other differences, including
-- whitespace/formatting.
--
-- The three additions (mirroring the existing selected_sponsor_domain_id
-- pattern exactly):
--   1. INSERT column list: + selected_marketer_domain_id
--   2. Outer SELECT: + guarded value (persisted only when use_marketer_domain = true, else null)
--   3. Inner lateral subquery: + parsing of elem->>'selected_marketer_domain_id'
--
-- Nothing else changed: no new validation, no auth changes, no RLS
-- changes, no signature changes (still 5 args, same order, same
-- defaults), same OID on replace. GRANT statements intentionally
-- omitted per instruction — this patch makes no permission changes.

CREATE OR REPLACE FUNCTION public.create_promotion(p_organization_id uuid, p_campaign_id uuid, p_asset_ids uuid[], p_assignment_collaborator_id uuid DEFAULT NULL::uuid, p_asset_usage jsonb DEFAULT NULL::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
begin
  ...
  if p_assignment_collaborator_id is not null
    ...
    insert into promotion_assets (
      ...
      asset_id,
      use_marketer_domain,
      selected_marketer_domain_id,   -- ★ ADDED
      selected_sponsor_domain_id,
      use_vstrk_domain
    )
    select
      ...
      coalesce(u.use_marketer_domain, false),
      case                                                      -- ★ ADDED
        when coalesce(u.use_marketer_domain, false)              -- ★ ADDED
        then u.selected_marketer_domain_id                       -- ★ ADDED
        else null                                                -- ★ ADDED
      end,                                                        -- ★ ADDED
      u.selected_sponsor_domain_id,
      coalesce(u.use_vstrk_domain, false)
    ...
    left join lateral (
      select
        coalesce((elem->>'use_marketer_domain')::boolean, false) as use_marketer_domain,
        case                                                              -- ★ ADDED
          when elem->>'selected_marketer_domain_id' is null                -- ★ ADDED
            or elem->>'selected_marketer_domain_id' = ''                   -- ★ ADDED
            or elem->>'selected_marketer_domain_id' = 'null'               -- ★ ADDED
          then null                                                        -- ★ ADDED
          else (elem->>'selected_marketer_domain_id')::uuid                -- ★ ADDED
        end as selected_marketer_domain_id,                                -- ★ ADDED
        case
          when elem->>'selected_sponsor_domain_id' is null
            or elem->>'selected_sponsor_domain_id' = ''
            or elem->>'selected_sponsor_domain_id' = 'null'
