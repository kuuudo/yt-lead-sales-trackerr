...
/** Campaign redirect link types the user may include when generating (Modal B). */
type CampaignLinkTypeKey = 'landing_page' | 'newsletter' | 'consultation' | 'sales_call';

const CAMPAIGN_LINK_TYPE_META: Record<
  CampaignLinkTypeKey,
  { label: string; campaignUrlKey: string }
> = {
  landing_page: { label: 'Direct purchase / Landing', campaignUrlKey: 'landing_page_url' },
  newsletter: { label: 'Newsletter', campaignUrlKey: 'newsletter_url' },
  consultation: { label: 'Consultation', campaignUrlKey: 'consultation_booking_url' },
...
function availableCampaignLinkTypes(campaign: any): CampaignLinkTypeKey[] {
  ...
  if (campaign.consultation_booking_url || campaign.has_paid_consultation) out.push('consultation');
  if (campaign.sales_call_booking_url || campaign.has_sales_call) out.push('sales_call');
  return out;
}
...
// Resolves the assignmentId for each promoted asset that currently has
  ...
  const applyEligiblePromotion = async (promotion: EligiblePromotion) => {
    ...
    try {
      ...
      // Global tracking domain: prefer first sponsor domain on assets
      ...
      if (firstSponsor?.selected_sponsor_domain_id) {
        ...
        setCreativeDomainOptions(
          firstSponsor.selected_sponsor_hostname
            ? [{ id: firstSponsor.selected_sponsor_domain_id, hostname: firstSponsor.selected_sponsor_hostname } as any]
            : []
        );
      } else {
        setCreativeDomainOptions([]);
                ...
                <div className="space-y-6">
                  <div className="space-y-1" data-tutorial-id="videos-platform-url">
                      ...
                      <div className="space-y-1" data-tutorial-id="videos-campaign">
                        ...
                        <select 
                          ...
                          onChange={e => {
                            ...
                            if (selectedCreativeAssignmentId) {
                          ...
                          {selectedCreativeAssignment ? (
                            <>
                              {selectedCreativeAssignment.onlyPromoteAssetCampaignId && (
                                <option value={selectedCreativeAssignment.onlyPromoteAssetCampaignId}>
                              ...
                              {selectedCreativeAssignment.creativeMode === 'campaign_links_and_assets' &&
                                selectedCreativeAssignment.creativeCampaignId && (
                                  <option value={selectedCreativeAssignment.creativeCampaignId}>
                        ...
                        {selectedCreativeAssignment && (
                          <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-500 pt-1">
                            {selectedCreativeAssignment.creativeMode === 'campaign_asset_only'
                              ? 'Creative · Campaign + asset only'
                              : 'Creative · Campaign + links + assets'}
                          </p>
                        )}
                        {selectedCreativeAssignment && creativeLinksUnavailable && (
                    ...
                    <div className="space-y-2" data-tutorial-id="videos-promoted-asset">
                      ...
                      ) : hasPromotionContext || selectedCreativePromotionId ? (
