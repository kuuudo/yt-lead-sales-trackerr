...
import { TrendingUp, Users, Loader2, ChevronRight, ChevronDown, X } from 'lucide-react';
...
// Combined shell for the two ranking cards. Owns tab state + the shared
// metric/date filters (switching tabs keeps whatever filter you had set).
// TopPromotions.tsx and TopMarketers.tsx are untouched and still work
// standalone if you need them elsewhere — this is a new, separate card.

type RankingTab = 'promotions' | 'marketers';
type Metric = 'revenue' | 'clicks' | 'sessions' | 'conversions' | 'rpc';
type Preset = 'last_7_days' | 'last_30_days' | 'this_month' | 'last_12_months' | 'all_time' | 'custom';
...
export default function TopRankings({ organizationId }: TopRankingsProps) {
  const navigate = useNavigate();

  const [tab, setTab] = useState<RankingTab>('promotions');
  ...
  const [promotionRows, setPromotionRows] = useState<TopPromotionRow[]>([]);
  const [marketerRows, setMarketerRows] = useState<MarketerRow[]>([]);
  ...
  useEffect(() => {
    ...
    const customRange =
      preset === 'custom' ? { start: new Date(customStart), end: new Date(customEnd) } : null;

    const request =
      tab === 'promotions'
        ? getTopPromotionsAnalytics({ organizationId, preset, customRange }).then(r => {
            if (!cancelled) setPromotionRows(r);
          })
        : getTopMarketersAnalytics({ organizationId, preset, customRange }).then(r => {
            if (!cancelled) setMarketerRows(r);
          });
  ...
  }, [organizationId, tab, preset, customStart, customEnd]);

  const rankedPromotions = useMemo(
    () => rankTopPromotions(promotionRows, metric),
    [promotionRows, metric],
  );
  const rankedMarketers = useMemo(
    () => rankTopMarketers(marketerRows, metric),
    [marketerRows, metric],
  );

  const ranked = tab === 'promotions' ? rankedPromotions : rankedMarketers;
  const visible = ranked.slice(0, INITIAL_VISIBLE);
  const hasMore = ranked.length > INITIAL_VISIBLE;

  const switchTab = (next: RankingTab) => {
    if (next === tab) return;
    setTab(next);
    setShowAll(false);
    setExpanded(new Set());
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-8">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-1 bg-zinc-950 border border-zinc-800 rounded-lg p-0.5">
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest transition-colors ${
              tab === 'promotions' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            <TrendingUp size={12} className={tab === 'promotions' ? 'text-red-500' : ''} />
            Top Promotions
          </button>
          <button
            onClick={() => switchTab('marketers')}
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest transition-colors ${
              tab === 'marketers' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            <Users size={12} className={tab === 'marketers' ? 'text-red-500' : ''} />
            Top Marketers
          </button>
        </div>
      ...
      {!loading && !error && visible.length === 0 && (
        <div className="text-zinc-600 text-xs text-center py-6">
          {tab === 'promotions' ? 'No promotion activity yet' : 'No marketer activity yet'}
        </div>
      )}
      ...
      {!loading && !error && visible.length > 0 && (
        <div className="divide-y divide-zinc-800/70">
          {tab === 'promotions'
            ? (visible as TopPromotionRow[]).map((row, i) => (
                <PromotionRankRow
                  key={row.promotionId}
                  rank={i + 1}
                  row={row}
            ...
            : (visible as MarketerRow[]).map((row, i) => (
                <MarketerRankRow
                  key={row.marketerId}
                  rank={i + 1}
                  row={row}
                  ...
                  onPromotionClick={id => navigate(`/marketplace/promotions/${id}`)}
                />
              ))}
        </div>
      )}
            ...
            >
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  ...
                  {tab === 'promotions' ? 'Top Promotions' : 'Top Marketers'}
                </h2>
                <button
                  onClick={() => setShowAll(false)}
                ...
                {tab === 'promotions'
                  ...
                  : rankedMarketers.map((row, i) => (
                      <MarketerRankRow
                        ...
                        onPromotionClick={id => {
                          setShowAll(false);
                          navigate(`/marketplace/promotions/${id}`);
                        }}
                      />
                    ))}
              </div>
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          {row.marketer && <p className="text-zinc-500 text-[11px] truncate">{row.marketer}</p>}
        </div>
      </div>
