...
/** Store video_id + campaign_id so every downstream call is attributed. */
...
export const syncSession = async () => {
  ...
  if (existingSessionId) {
    // Session row already exists. If it was created before attribution was
    // set (e.g. by useTracker firing before Track.tsx ran), patch it now so
    // the row is never left with null video_id / campaign_id.
    // Only update when we actually have attribution data to write.
    if (videoId || campaignId) {
      ...
      const { error } = await supabase
        ...
        // Only patch if columns are still null — avoids overwriting
        // legitimate attribution on a returning visitor's session.
        .is('video_id', null);

      if (error) {
        console.warn('[tracker] syncSession patch failed:', error.message);
      } else {
        console.debug('[tracker] syncSession patched existing session', { existingSessionId, ...patch });
      }
    }
    return;
  }

  // No session yet — create one with full attribution from the start.
